...
class ParametricCurveExplainer(BaseExplainer):
    ...
    # BaseExplainer interface methods
    def explain_concepts(self) -> str:
        return self.conceptual_overview().replace("## Conceptual overview\n", "", 1)

    def explain_inputs(self) -> str:
        return self.data_requirements_and_preprocessing().replace(
            "## Data requirements and preprocessing assumptions\n", "", 1
        )

    def explain_calibration(self) -> str:
        return self.calibration_caveats().replace("## Calibration caveats\n", "", 1)

    def explain_outputs(self) -> str:
        return self.mathematical_form_and_parameter_intuition().replace(
            "## Mathematical form and parameter intuition\n", "", 1
        )

    def explain_trading_implications(self) -> str:
        return self.trading_interpretation().replace("## Output interpretation for trading\n", "", 1)
